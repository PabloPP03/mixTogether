      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto bg-white p-6 rounded shadow mb-10 font-body">
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={resetForm} className="font-buttons px-6 py-2 rounded border">
              Cancelar
            </button>
            <button type="submit" className="font-buttons px-6 py-2 rounded font-semibold" style={{ backgroundColor: COLORS.primary, color: COLORS.card }}>
              Guardar
            </button>
          </div>
        </form>
        ...
        {mezclas.map((m) => (
          <div key={m.id} className="bg-white rounded shadow p-4 font-body">
            ...
            <div className="flex gap-2">
              <button onClick={() => handleEdit(m)} className="font-buttons text-sm px-3 py-1 rounded border">Editar</button>
              <button onClick={() => handleDelete(m.id)} className="font-buttons text-sm px-3 py-1 rounded border text-red-600">Borrar</button>
            </div>
          </div>
        ))}
      </div>
